...
import { Mail, Lock, Eye, EyeOff, ArrowRight, X, Sparkles, Globe, AlertTriangle } from "lucide-react";
import { ScreenID } from "../types";
import { supabase, loginUser, syncGoogleUserProfile } from "../lib/supabaseService";
...
export default function LoginScreen({ onNavigate, setUserName }: LoginScreenProps) {
  ...
  // Check if a Google account has previously been used on this device
  useEffect(() => {
    const savedUser = localStorage.getItem("studentos_google_user");
    if (savedUser) {
      try {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      const userName = googleUser?.name || "Tanusree";
      const userEmail = googleUser?.email || "samantatanusree2005@gmail.com";
      const profilePhoto = googleUser?.profilePhoto || "https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=150";
      const uid = googleUser?.uid || "google-mock-uid-123";
      ...
      // Sync user profile in Supabase table
      const isNewGoogleUser = await syncGoogleUserProfile(mockUserObj);

      if (isNewGoogleUser) {
        localStorage.setItem(
          "studentos_welcome_msg",
          `Welcome, ${userName}! Your StudentOS account is ready.`
        );
      }
      ...
      console.warn("Google sign in error:", errMsg);
      setErrorMessage("Google sign in failed: " + errMsg);
      setIsAuthenticating(false);
    }
  };
      ...
      <div className="w-full max-w-md bg-white/95 backdrop-blur-md border border-white/20 p-8 sm:p-10 rounded-3xl shadow-2xl relative z-10">
        ...
        <div className="mb-6">
          <button
            ...
            className="w-full bg-white hover:bg-slate-50 text-slate-700 border border-slate-200 rounded-2xl p-3.5 flex items-center justify-center transition-all cursor-pointer shadow-sm font-bold text-xs gap-3 hover:border-slate-300 focus:outline-none focus:ring-2 focus:ring-indigo-500/10 disabled:opacity-75"
          >
